...
export class ArtworkModel {
  ...
  static async findById(id: number): Promise<ArtworkCollection | null> {
    return await getRow(
      'SELECT * FROM artwork_collections WHERE id = $1',
      [id]
    )
  }

  // 获取所有作品集（分页）
  static async findAll(page: number = 1, limit: number = 10, status: string = 'active'): Promise<{
  ...
  // 更新作品集
  static async update(id: number, data: UpdateArtworkData): Promise<ArtworkCollection | null> {
    const fields = []
    const values = []
    let paramIndex = 1
  ...
  // 记录点赞
  static async recordLike(collectionId: number, ipAddress: string, userAgent?: string): Promise<boolean> {
    try {
      await query(
  ...
  // 检查是否已点赞
  static async hasLiked(collectionId: number, ipAddress: string): Promise<boolean> {
  ...
  // 添加图片到作品集
  static async addImage(collectionId: number, imageData: {
    filename: string
    original_name: string
    file_url: string
    thumbnail_url?: string
  ...
  // 更新图片排序
  static async updateImageOrder(imageId: number, sortOrder: number): Promise<void> {
    await query(
      'UPDATE artwork_images SET sort_order = $1 WHERE id = $2',
      [sortOrder, imageId]
    )
  }

  // 根据标签搜索作品集
  static async findByTags(tags: string[], page: number = 1, limit: number = 10): Promise<{
    collections: ArtworkCollection[]
    total: number
    totalPages: number
    currentPage: number
