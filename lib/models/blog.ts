...
export class BlogModel {
  ...
  static async findBySlug(slug: string): Promise<BlogPost | null> {
    ...
    const result = await db.query(
      "SELECT * FROM blog_posts WHERE slug = $1 OR slug = $2 LIMIT 1",
      [nfc, nfd],
    );
    return result.rows[0] || null;
  }
  ...
  static async createTag(
    tag: Omit<BlogTag, "id" | "created_at">,
  ): Promise<BlogTag> {
    const result = await db.query(
  ...
  static async findTagBySlug(slug: string): Promise<BlogTag | null> {
    const { nfc, nfd } = normalizeVariants(slug);
    const result = await db.query(
      "SELECT * FROM blog_tags WHERE slug = $1 OR slug = $2 LIMIT 1",
      [nfc, nfd],
    );
    return result.rows[0] || null;
  }
  ...
  static async createComment(
    comment: Omit<BlogComment, "id" | "created_at" | "updated_at">,
  ): Promise<BlogComment> {
    const result = await db.query(
  ...
  static async getPostComments(
    postId: number,
    status: "approved" | "pending" | "spam" = "approved",
  ): Promise<BlogComment[]> {
    const result = await db.query(
