...
export class LikesModel {
  static async getLikesCount(
    ...
    if (targetType === "blog") {
      ...
      const res = await query(
        "SELECT likes_count FROM artwork_collections WHERE id = $1",
        [targetId],
      );
      return res.rows[0]?.likes_count ?? 0;
    } else if (targetType === "music") {
      const res = await query(
        "SELECT likes_count FROM music_tracks WHERE id = $1",
        [targetId],
      );
      return res.rows[0]?.likes_count ?? 0;
    } else if (targetType === "video") {
      const res = await query("SELECT likes_count FROM videos WHERE id = $1", [
        targetId,
      ]);
      return res.rows[0]?.likes_count ?? 0;
    }
  ...
  static async setLike(
    ...
    if (existing) {
      await query(`UPDATE likes SET status = $1 WHERE id = $2`, [
        status,
        existing.id,
      ]);
    } else {
