...
export class AppModel {
  ...
  // 记录点赞
  static async recordLike(appId: number, ipAddress: string, userAgent?: string): Promise<boolean> {
    try {
      await query(
  ...
  static async incrementLikes(id: number): Promise<void> {
    await query(
      'UPDATE apps SET likes_count = likes_count + 1 WHERE id = $1',
      [id]
    )
  }
  ...
  // 更新每日统计
  static async updateDailyStats(appId: number, date: string, stats: Partial<AppStats>): Promise<void> {
    await query(
  ...
  static async getTags(): Promise<AppTag[]> {
    return await getRows(
      'SELECT * FROM app_tags ORDER BY name ASC'
    )
  }

  // 创建标签
  static async createTag(data: { name: string; description?: string; color?: string }): Promise<AppTag> {
    const slug = generateSlug(data.name)
    const result = await query(
  ...
  // 搜索应用
  static async search(queryText: string, options: {
    page?: number
    limit?: number
    type?: string
    platform?: string
    ...
    const offset = (page - 1) * limit
    const conditions = ['status = $1', '(name ILIKE $2 OR description ILIKE $2)']
