...
export class VideoModel {
  ...
  static async findAll(page = 1, limit = 10, status = "active") {
    ...
    const count = await query("SELECT COUNT(*) FROM videos WHERE status = $1", [
      status,
    ]);
  ...
  static async update(
    id: number,
    data: Partial<CreateVideoData & { status: string }>,
  ): Promise<Video | null> {
    const fields = [];
    const values = [];
    let paramCount = 1;
  ...
  static async updateLikes(id: number, likes: number) {
    await query(
      "UPDATE videos SET likes_count = $1, updated_at = CURRENT_TIMESTAMP WHERE id = $2",
      [likes, id],
    );
  }
}
