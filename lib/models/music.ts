...
export class MusicModel {
  static async create(data: CreateMusicData): Promise<MusicTrack> {
    const res = await query(
      `INSERT INTO music_tracks (title, tags, audio_url, cover_url, duration)
       VALUES ($1, $2, $3, $4, $5)
       RETURNING *`,
      [data.title, data.tags, data.audio_url, data.cover_url, data.duration || 0]
    )
    return res.rows[0]
  }
  ...
  static async findAll(page: number = 1, limit: number = 10, status: string = 'active') {
    const offset = (page - 1) * limit
    const count = await query('SELECT COUNT(*) FROM music_tracks WHERE status = $1', [status])
  ...
  static async updateLikes(id: number, likes: number) {
    await query('UPDATE music_tracks SET likes_count = $1, updated_at = CURRENT_TIMESTAMP WHERE id = $2', [likes, id])
  }
}
