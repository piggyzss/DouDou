...
export function readPostFile(slug: string): { frontmatter: BlogFrontmatter; content: string } | null {
  ensureBlogDirExists()
...
export function getPostBySlug(slug: string): (BlogPostMeta & { content: string }) | null {
...
export function paginatePosts(posts: BlogPostMeta[], page: number, perPage: number) {
...
export function saveNewPost(params: { title: string; slug: string; tags: string[]; content: string; date?: string }) {
...
export function updateExistingPost(params: { slug: string; title: string; tags: string[]; content: string; date?: string }) {
  if (process.env.NODE_ENV !== 'development') {
  ...
  const frontmatter: BlogFrontmatter = {
    title: params.title,
    date: params.date ?? (existing ? normalizeFrontmatter(matter.read(filePath).data).date : new Date().toISOString()),
    tags: params.tags,
  }
  const content = matter.stringify(params.content, frontmatter)
  fs.writeFileSync(filePath, content, 'utf8')
