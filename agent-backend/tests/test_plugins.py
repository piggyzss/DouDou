...
async def test_news_plugin_latest_command(news_plugin, sample_request):
    ...
    with patch.object(news_plugin.news_service, 'get_latest_news', new_callable=AsyncMock) as mock_get_news:
...
async def test_news_plugin_trending_command(news_plugin):
    ...
    with patch.object(news_plugin.news_service, 'get_trending_topics', new_callable=AsyncMock) as mock_get_trending:
