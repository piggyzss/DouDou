...
export async function POST(request: NextRequest) {
  try {
    ...
    const experienceMethod = formData.get('experience_method') as 'download' | 'qrcode'
    const downloadUrl = formData.get('download_url') as string
    const experienceUrl = formData.get('experience_url') as string

    // 验证必填字段
    if (!name || !description || !type || !platform || !status || !experienceMethod) {
      return NextResponse.json(
        { error: '缺少必填字段' },
        { status: 400 }
      )
    ...
    if (coverImage && coverImage.size > 0) {
      try {
        ...
        const uploadResult = await uploadFile(coverBuffer, coverImage.name, coverImage.type, 'apps/covers/')
    ...
    if (video && video.size > 0) {
      try {
        ...
        const uploadResult = await uploadFile(videoBuffer, video.name, video.type, 'apps/videos/')
        if (uploadResult.success && uploadResult.url) {
    ...
    if (experienceMethod === 'qrcode') {
      ...
      if (qrCodeImage && qrCodeImage.size > 0) {
        try {
          ...
          const uploadResult = await uploadFile(qrBuffer, qrCodeImage.name, qrCodeImage.type, 'apps/qr-codes/')
          if (uploadResult.success && uploadResult.url) {
