...
export async function POST(req: NextRequest) {
  try {
    ...
    const ip =
      (req as any).ip || req.headers.get("x-forwarded-for") || "0.0.0.0";
    ...
    // 获取用户当前状态和总数
    const userLiked = await getUserLikeStatus(
      targetType,
      idNum,
      ipHash,
      uaHash,
...
async function updateLikesCount(targetType: string, targetId: number) {
  try {
    ...
    if (targetType === "blog") {
      await query("UPDATE blog_posts SET likes_count = $1 WHERE id=$2", [
        actualCount,
        targetId,
      ]);
    } else if (targetType === "artwork") {
      await query(
        "UPDATE artwork_collections SET likes_count = $1 WHERE id=$2",
        [actualCount, targetId],
      );
    } else if (targetType === "music") {
      await query("UPDATE music_tracks SET likes_count = $1 WHERE id=$2", [
        actualCount,
        targetId,
      ]);
    } else if (targetType === "video") {
      await query("UPDATE videos SET likes_count = $1 WHERE id=$2", [
        actualCount,
        targetId,
      ]);
    }
