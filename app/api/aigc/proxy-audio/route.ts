...
export async function GET(req: NextRequest) {
  try {
    ...
    const isCosHost =
      hostname.includes(".cos.") && hostname.endsWith(".myqcloud.com");
    if (!isCosHost) {
      const range = req.headers.get("range") || undefined;
      const res = await fetch(url, {
        headers: range ? ({ range } as any) : undefined,
      });
      if (!res.ok || !res.body)
    ...
    const headers: Record<string, string> = {
      "content-type":
        (result.headers?.["content-type"] as string) || "audio/mpeg",
      "accept-ranges": "bytes",
      "cache-control": "public, max-age=300",
    };
    const contentRange = result.headers?.["content-range"] as
      | string
      | undefined;
