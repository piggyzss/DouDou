...
export default function FloatingPlayerBar({
  title,
  tags,
  coverUrl,
  /* likes */ isPlaying,
  ...
  return (
    <div className="fixed left-0 right-0 bottom-4 md:bottom-6 flex justify-center z-40">
      <div className="relative w-[92%] md:w-1/2 mx-auto rounded overflow-visible hover:shadow-lg transition-all duration-300 border border-gray-200 dark:border-gray-700 player-pop">
          ...
          <div className="relative py-3.5 md:py-4 px-2.5 z-10">
            ...
            <div className="flex items-center gap-3 md:gap-3 py-2">
                ...
                {isPlaying && (
                  <div className="absolute inset-0 flex items-center justify-center">
                    <span className="p-1 bg-white bg-opacity-90 rounded-full shadow-lg">
                      <Headphones
                        size={14}
                        className="text-gray-600/70 animate-[spin_2.5s_linear_infinite]"
                      />
                    </span>
                  </div>
                )}
              ...
              <div className="min-w-0 flex-1">
                <div className="text-sm md:text-base font-heading text-text-primary truncate">
                  {title}
                </div>
                <div className="flex flex-wrap items-center gap-1 mt-1">
                  {tags.slice(0, 3).map((tag) => (
                    <span
                      key={tag}
                      className="px-2 py-0.5 rounded-full bg-gray-100 dark:bg-gray-700 text-text-secondary text-[10px] md:text-xs font-blog"
                    >
                      #{tag}
                    </span>
                  ))}
              ...
              <div className="flex items-center justify-center gap-2.5 flex-none">
                <button
                  onClick={onPrev}
                  className="h-8 w-8 flex items-center justify-center rounded transition-transform duration-150 hover:scale-105 active:scale-95"
                  title="上一首"
                >
                  <SkipBack size={16} />
                </button>
              ...
              <div className="flex items-center justify-end gap-2 md:gap-2.5 flex-1">
                ...
                >
                  ...
                  <div
                    className={`overflow-hidden transition-all duration-300 ease-out relative ${volOpen ? "w-28 opacity-100" : "w-0 opacity-0"}`}
                  >
                    <span
        ...
        {/* 进度条：纯色 --primary */}
        <div
          className="absolute left-0 right-0 bottom-0 h-1.5 cursor-pointer rounded-b-lg overflow-hidden"
