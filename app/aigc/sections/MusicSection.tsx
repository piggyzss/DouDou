...
export default function MusicSection({ tracks, currentTrackId, setCurrentTrackId, isAudioPlaying, formatDuration, formatNumber, formatDate, onDeleteTrack, onUpdateMusicLikes }: MusicSectionProps) {
  ...
  const playTrack = (track: MusicTrack) => {
    window.dispatchEvent(new CustomEvent('music:play', { detail: { url: track.audioUrl, id: track.id } }))
  }
  ...
  if (!tracks || tracks.length === 0) {
    return (
      <div className="text-center py-12">
        ...
        <p className="text-sm text-text-muted mt-2 blog-body-text">点击上方按钮创建您的第一首音乐</p>
      </div>
    )
  }
  ...
  return (
    <div className="divide-y divide-gray-200 dark:divide-gray-800 bg-transparent">
      {tracks.map((track) => {
        ...
        const overlayIcon = isCurrent && isAudioPlaying ? (
          <Pause size={16} />
        ) : (
          <Play size={16} />
        )
          ...
          <div key={track.id} className="flex items-center gap-4 py-4 group hover:bg-gray-50/40 dark:hover:bg-gray-800/40 rounded-lg px-2 transition-colors">
            ...
            <div className="min-w-0 flex-1">
              ...
              <div className="mt-1.5 md:mt-2 flex flex-wrap items-center gap-2">
                <span className="text-[11px] text-text-muted">{formatDate(track.createdAt)}</span>
                ...
                {track.tags.slice(0, 4).map(tag => (
                  <span key={tag} className="px-2 py-0.5 rounded-full bg-gray-100 dark:bg-gray-700 text-text-secondary text-[11px] md:text-xs font-blog">#{tag}</span>
                ))}
              </div>
            </div>
            ...
            <div className="flex items-center gap-4 ml-2">
              <div className="flex items-center gap-1 text-[11px] text-text-muted">
                <LikeToggle
                  ...
                  onChanged={(_, count) => { onUpdateMusicLikes?.(track.id, count) }}
                />
              </div>
              <div className="flex items-center gap-1 text-[11px] text-text-muted">
                <MessageCircle size={14} className="text-text-muted" />
                <span className="leading-none text-text-muted text-[11px]">{formatNumber(track.comments)}</span>
              </div>
            </div>
          </div>
        )
