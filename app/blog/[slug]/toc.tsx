...
export default function TOC() {
  ...
  useEffect(() => {
    const nodes = Array.from(document.querySelectorAll('article h1, article h2, article h3, article h4, article h5, article h6')) as HTMLElement[]
    const hs = nodes.map((el) => {
      const level = Number(el.tagName.substring(1))
      const text = el.textContent || ''
      const id = el.id || text.toLowerCase().replace(/[^a-z0-9\u4e00-\u9fa5\s-]/g, '').replace(/\s+/g, '-')
    ...
    <nav className="text-sm font-blog">
      <div className="bg-gray-50 dark:bg-gray-800 rounded-lg p-4">
        ...
        <ul className="space-y-2">
          {headings.map((h) => (
            <li key={h.id} className={`truncate ${activeId === h.id ? 'text-primary font-medium' : 'text-text-secondary'}`}>
              <a
