...
export default async function BlogDetailPage({ params }: Params) {
  const normalizedSlug = (() => {
    try {
      return decodeURIComponent(params.slug).normalize("NFC");
    } catch {
      return String(params.slug || "").normalize("NFC");
    }
  })();
            ...
            <article className="min-w-0">
              ...
              {(post as any).cover_url && (
                <div className="mt-4">
                  <Image
                    src={
                      (post as any).cover_url.startsWith("/")
                        ? (post as any).cover_url
                        : `/api/aigc/proxy-image?url=${encodeURIComponent((post as any).cover_url)}`
                    }
                    alt={post.title}
                    width={800}
                    height={360}
