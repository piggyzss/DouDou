...
export default function Update() {
  ...
  const [rabbitDirection, setRabbitDirection] = useState<'left-to-right' | 'right-to-left'>('left-to-right')
  ...
  const generateMosaicTiles = (): MosaicTile[] => {
    ...
    for (let row = 0; row < rows; row++) {
      for (let col = 0; col < cols; col++) {
        ...
        const positionInfluence = Math.sin(actualCol * 0.15) * Math.cos(row * 0.2) * 0.5 + 0.5
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      ...
      if (targetTile) {
        ...
        setRabbitDirection(shouldBeLeftToRight ? 'left-to-right' : 'right-to-left')
