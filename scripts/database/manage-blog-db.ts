  ...
  try {
    ...
    if (comments.length > 0) {
      ...
      comments.forEach((comment: any, index: number) => {
        console.log(
          `  ${index + 1}. ${comment.author_name} (${comment.status})`,
        );
...
async function showDatabaseStats() {
  try {
    ...
    const approvedCommentsResult = await query(
      "SELECT COUNT(*) as count FROM blog_comments WHERE status = 'approved'",
    );
    const approvedCommentsCount = approvedCommentsResult.rows[0].count;

    // 总浏览量和点赞数
    const totalViewsResult = await query(
      "SELECT SUM(views_count) as total FROM blog_posts",
    );
    const totalViews = totalViewsResult.rows[0].total || 0;

    const totalLikesResult = await query(
      "SELECT SUM(likes_count) as total FROM blog_posts",
    );
    ...
    if (recentPosts.rows.length > 0) {
      ...
      recentPosts.rows.forEach((row: any) => {
        console.log(
          `  - ${row.title} (${row.status}) - ${row.views_count} 浏览`,
        );
        const createdAtDate =
