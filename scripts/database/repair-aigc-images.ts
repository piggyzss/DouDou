...
async function main() {
  ...
  console.log(
    `🔎 扫描 AIGC 图片有效性（${DRY_RUN ? "预览模式" : "删除无效记录"}）`,
  );
  try {
    const rows = (await db.getRows(
      "SELECT id, file_url FROM artwork_images ORDER BY id ASC",
    )) as ImageRow[];
    ...
    if (missing.length > 0) {
      ...
      missing
        .slice(0, 10)
        .forEach((m) => console.log(`  - id=${m.id} url=${m.file_url}`));

      if (!DRY_RUN) {
        // 执行删除缺失记录
        const ids = missing.map((m) => m.id);
        const res = await db.query(
          "DELETE FROM artwork_images WHERE id = ANY($1::int[])",
          [ids],
        );
        console.log(`🧹 已删除缺失记录: ${res.rowCount || 0}`);
      } else {
        console.log(
          "💡 预览模式未删除任何记录。要删除请以 FIX=delete 环境变量运行本脚本。",
        );
      }
    }
  } catch (e) {
