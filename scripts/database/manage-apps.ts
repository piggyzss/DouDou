...
async function listAllApps() {
  try {
    ...
    result.apps.forEach((app: any) => {
      ...
      console.log(`类型: ${app.type} | 平台: ${app.platform} | 状态: ${app.status}`)
      console.log(`体验方式: ${app.experience_method}`)
      console.log(`标签: ${app.tags?.join(', ') || '无'}`)
      console.log(`DAU: ${app.dau} | 下载量: ${app.downloads} | 点赞数: ${app.likes_count}`)
...
async function filterAppsByStatus() {
  try {
    ...
    result.apps.forEach((app: any) => {
      console.log(`ID: ${app.id} | 名称: ${app.name} | 类型: ${app.type} | 平台: ${app.platform}`)
      console.log(`DAU: ${app.dau} | 下载量: ${app.downloads} | 点赞数: ${app.likes_count}`)
      console.log(`创建时间: ${app.created_at}`)
...
async function filterAppsByType() {
  try {
    ...
    result.apps.forEach((app: any) => {
      console.log(`ID: ${app.id} | 名称: ${app.name} | 状态: ${app.status} | 平台: ${app.platform}`)
      console.log(`DAU: ${app.dau} | 下载量: ${app.downloads} | 点赞数: ${app.likes_count}`)
      console.log(`创建时间: ${app.created_at}`)
      console.log('-'.repeat(40))
    })
...
async function filterAppsByPlatform() {
  try {
    ...
    result.apps.forEach((app: any) => {
      console.log(`ID: ${app.id} | 名称: ${app.name} | 类型: ${app.type} | 状态: ${app.status}`)
      console.log(`DAU: ${app.dau} | 下载量: ${app.downloads} | 点赞数: ${app.likes_count}`)
      console.log(`创建时间: ${app.created_at}`)
      console.log('-'.repeat(40))
    })
...
async function updateApp() {
  try {
    ...
    const newStatus = await new Promise<string>((resolve) => {
      readline.question(`状态 (online/beta/development) [${app.status}]: `, resolve)
    })
    ...
    const newTrend = await new Promise<string>((resolve) => {
      readline.question(`趋势 (rising/stable/declining) [${app.trend}]: `, resolve)
    })
    ...
    if (newDownloads.trim()) updateData.downloads = parseInt(newDownloads.trim())
...
async function showDatabaseStats() {
  try {
    ...
    statusStats.rows.forEach((row: any) => {
      const statusName = row.status === 'online' ? '已上线' : 
                        row.status === 'beta' ? '测试版' : '开发中'
      console.log(`  ${statusName}: ${row.count} 个`)
    })

    console.log('\n按类型统计:')
    typeStats.rows.forEach((row: any) => {
      const typeName = row.type === 'app' ? '应用' : 
                      row.type === 'miniprogram' ? '小程序' : '游戏'
      console.log(`  ${typeName}: ${row.count} 个`)
    })

    console.log('\n按平台统计:')
    platformStats.rows.forEach((row: any) => {
      const platformName = row.platform === 'web' ? 'Web' : 
                          row.platform === 'mobile' ? '移动端' : '微信'
    ...
    if (recentApps.rows.length > 0) {
      ...
      recentApps.rows.forEach((row: any) => {
        console.log(`  - ${row.name} (${row.type}/${row.platform}/${row.status})`)
...
async function showTableStructure() {
  try {
    ...
    // 检查点赞表
    try {
      const likesCount = await query("SELECT COUNT(*) as count FROM likes WHERE target_type = 'app'")
