...
async function updateVideo() {
  try {
    ...
    const newTagsStr = await askQuestion(
      `新标签 (用逗号分隔, 当前: ${video.tags ? video.tags.join(", ") : "无"}): `,
    );
    const newStatus = await askQuestion(
      `新状态 (active/draft/archived, 当前: ${video.status}): `,
    );
...
async function showStatistics() {
  try {
    ...
    if (durationResult.rows[0].avg_duration) {
      ...
      console.log("\n⏱️ 时长统计:");
      console.log(
        `  平均时长: ${formatDuration(Math.round(stats.avg_duration))}`,
      );
...
async function filterByTag() {
  try {
    ...
    result.rows.forEach((video: VideoRecord, index: number) => {
      ...
      console.log(
        `   时长: ${formatDuration(video.duration)} | 点赞: ${video.likes_count}`,
      );
