  ...
  try {
    ...
    if (images.length > 0) {
      ...
      images.forEach((img: ImageRecord, index: number) => {
        console.log(
          `  ${index + 1}. ${img.original_name} (${img.file_size} bytes)`,
        );
...
async function updateCollection() {
  try {
    ...
    const status = await new Promise<string>((resolve) => {
      readline.question(
        "新状态 (active/draft/archived，留空保持不变): ",
        resolve,
      );
    });
...
async function showDatabaseStats() {
  try {
    ...
    const collectionsResult = await query(
      "SELECT COUNT(*) as count FROM artwork_collections",
    );
    const collectionsCount = collectionsResult.rows[0].count;

    const activeCollectionsResult = await query(
      "SELECT COUNT(*) as count FROM artwork_collections WHERE status = 'active'",
    );
    const activeCollectionsCount = activeCollectionsResult.rows[0].count;

    // 图片统计
    const imagesResult = await query(
      "SELECT COUNT(*) as count FROM artwork_images",
    );
    ...
    const totalLikesResult = await query(
      "SELECT SUM(likes_count) as total FROM artwork_collections",
    );
    ...
    if (recentCollections.rows.length > 0) {
      ...
      recentCollections.rows.forEach((row: any) => {
        console.log(
          `  - ${row.title} (${row.created_at.split("T")[0]}) - ${row.likes_count} 赞`,
        );
      });
    }
  } catch (error) {
