...
async function main() {
  ...
  try {
    ...
    if (!process.env.DATABASE_URL && !process.env.POSTGRES_URL) {
      ...
      console.log(
        "💡 请创建 .env.local 文件并配置 DATABASE_URL 或 POSTGRES_URL",
      );
    ...
    try {
      const img = (await db.getRow(
        "SELECT file_url FROM artwork_images ORDER BY id ASC LIMIT 1",
      )) as { file_url?: string };
      if (img?.file_url) {
