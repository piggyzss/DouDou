...
async function testAppsAPI() {
  try {
    ...
    // 测试6: 记录点赞
    console.log("\n6. 测试记录点赞...");
    const likeResult = await AppModel.recordLike(
      newApp.id,
      "127.0.0.1",
      "Test User Agent",
    );
    ...
    // 测试9: 更新每日统计
    console.log("\n9. 测试更新每日统计...");
    await AppModel.updateDailyStats(
      newApp.id,
    ...
    // 测试14: 按平台筛选
    console.log("\n14. 测试按平台筛选...");
    const platformResult = await AppModel.findAll({
      platform: "web",
      page: 1,
      limit: 5,
    });
    ...
    // 测试15: 按标签筛选
    console.log("\n15. 测试按标签筛选...");
    const tagResult = await AppModel.findAll({
      tag: "测试",
      page: 1,
      limit: 5,
    });
