...
jest.mock('framer-motion', () => ({
  motion: {
    nav: ({ children, ...props }: any) => {
      ...
      return <nav data-testid="motion-nav" {...restProps}>{children}</nav>
    },
    ...
    button: ({ children, ...props }: any) => {
      const { whileHover, whileTap, layoutId, initial, animate, variants, transition, ...restProps } = props
      return <button data-testid="motion-button" {...restProps}>{children}</button>
    },
    form: ({ children, ...props }: any) => {
      const { whileHover, whileTap, layoutId, initial, animate, variants, transition, ...restProps } = props
      return <form data-testid="motion-form" {...restProps}>{children}</form>
    },
    input: ({ ...props }: any) => {
      const { whileHover, whileTap, layoutId, initial, animate, variants, transition, ...restProps } = props
...
describe('Navigation Component', () => {
  ...
  it('shows mobile menu when hamburger is clicked', async () => {
    ...
    await waitFor(() => {
      const mobileMenu = screen.getAllByTestId('motion-div').find(div => 
        div.className.includes('md:hidden')
      )
  ...
  it('closes mobile menu when X button is clicked', async () => {
    ...
    await waitFor(() => {
      const mobileMenus = screen.getAllByTestId('motion-div').filter(div => 
        div.className.includes('md:hidden')
      )
