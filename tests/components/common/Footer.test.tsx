...
describe('Footer Component', () => {
  it('renders footer with personal information', () => {
    ...
    expect(screen.getByText('前端开发者 | AI爱好者 | 创意工作者')).toBeInTheDocument()
    expect(screen.getByText('用代码创造美好，用AI探索未来，让创意与技术完美融合')).toBeInTheDocument()
  })
  ...
  it('renders current year in copyright', () => {
    ...
    expect(screen.getByText(`© ${currentYear} shanshan. 保留所有权利.`)).toBeInTheDocument()
  })
  ...
  it('has correct href attributes for social links', () => {
    ...
    const allLinks = screen.getAllByRole('link')
    const githubLink = allLinks.find(link => link.getAttribute('href')?.includes('github.com'))
    const twitterLink = allLinks.find(link => link.getAttribute('href')?.includes('twitter.com'))
    const linkedinLink = allLinks.find(link => link.getAttribute('href')?.includes('linkedin.com'))
    ...
    expect(twitterLink).toHaveAttribute('href', 'https://twitter.com/yourusername')
  ...
  it('has proper grid layout classes', () => {
    ...
    const gridContainer = screen.getByText('<shanshan />').closest('div')?.parentElement?.parentElement
